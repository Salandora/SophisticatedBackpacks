...
public class DepositFilterLogic extends FilterLogic {
	...
	private void setFilterByInventory(boolean filterByInventory) {
		upgrade.sophisticatedFabricLibrary_set(ModDataComponents.FILTER_BY_INVENTORY, filterByInventory);
		save();
	}

	private boolean shouldFilterByInventory() {
		return upgrade.sophisticatedFabricLibrary_getOrDefault(ModDataComponents.FILTER_BY_INVENTORY, false);
	}
}
