...
public class SmithingUpgradeTab extends UpgradeSettingsTab<SmithingUpgradeContainer> {
	...
	protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		int inputSlotsY = resultSlot.y + screen.sophisticatedFabricLibrary_getGuiTop();
		int additionalSlotX = getContainer().getAdditionalSlot().x + screen.sophisticatedFabricLibrary_getGuiLeft();
	...
	protected void moveSlotsToTab() {
		...
		templateSlot.x = x - screen.sophisticatedFabricLibrary_getGuiLeft() + 4;
		templateSlot.y = y - screen.sophisticatedFabricLibrary_getGuiTop() + 1 + 24;

		Slot baseSlot = getContainer().getBaseSlot();
		baseSlot.x = templateSlot.x + 18;
		baseSlot.y = y - screen.sophisticatedFabricLibrary_getGuiTop() + 1 + 24;

		Slot additionalSlot = getContainer().getAdditionalSlot();
		additionalSlot.x = baseSlot.x + 18;
		additionalSlot.y = y - screen.sophisticatedFabricLibrary_getGuiTop() + 1 + 24;

		Slot resultSlot = getContainer().getResultSlot();
		resultSlot.x = x - screen.sophisticatedFabricLibrary_getGuiLeft() + getWidth() - 2 - 3 - 18;
		resultSlot.y = y - screen.sophisticatedFabricLibrary_getGuiTop() + 1 + 24;
	}

	private boolean isHoveringRedCross(int mouseX, int mouseY) {
		Slot additionalSlot = getContainer().getAdditionalSlot();
		int arrowX = getArrowX(additionalSlot.x + screen.sophisticatedFabricLibrary_getGuiLeft(), getContainer().getResultSlot().x + screen.sophisticatedFabricLibrary_getGuiLeft());
		int arrowY = getArrowY(additionalSlot.y + screen.sophisticatedFabricLibrary_getGuiTop());
	...
	private boolean isHoveringEmptySlot(Slot slot, int mouseX, int mouseY) {
		return mouseX >= slot.x + screen.sophisticatedFabricLibrary_getGuiLeft() && mouseX < slot.x + screen.sophisticatedFabricLibrary_getGuiLeft() + 16 && mouseY >= slot.y + screen.sophisticatedFabricLibrary_getGuiTop() && mouseY < slot.y + screen.sophisticatedFabricLibrary_getGuiTop() + 16 && slot.getItem().isEmpty();
	}
