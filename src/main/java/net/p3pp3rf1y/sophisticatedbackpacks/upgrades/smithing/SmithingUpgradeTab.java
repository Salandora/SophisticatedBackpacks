...
public class SmithingUpgradeTab extends UpgradeSettingsTab<SmithingUpgradeContainer> {
	...
	protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		int inputSlotsY = resultSlot.y + ((AbstractContainerScreenAccessor) screen).getGuiTop();
		int additionalSlotX = getContainer().getAdditionalSlot().x + ((AbstractContainerScreenAccessor) screen).getGuiLeft();
	...
	protected void moveSlotsToTab() {
		...
		((SlotAccessor) templateSlot).setX(x - ((AbstractContainerScreenAccessor) screen).getGuiLeft() + 4);
		((SlotAccessor) templateSlot).setY(y - ((AbstractContainerScreenAccessor) screen).getGuiTop() + 1 + 24);

		Slot baseSlot = getContainer().getBaseSlot();
		((SlotAccessor) baseSlot).setX(templateSlot.x + 18);
		((SlotAccessor) baseSlot).setY(y - ((AbstractContainerScreenAccessor) screen).getGuiTop() + 1 + 24);

		Slot additionalSlot = getContainer().getAdditionalSlot();
		((SlotAccessor) additionalSlot).setX(baseSlot.x + 18);
		((SlotAccessor) additionalSlot).setY(y - ((AbstractContainerScreenAccessor) screen).getGuiTop() + 1 + 24);

		Slot resultSlot = getContainer().getResultSlot();
		((SlotAccessor) resultSlot).setX(x - ((AbstractContainerScreenAccessor) screen).getGuiLeft() + getWidth() - 2 - 3 - 18);
		((SlotAccessor) resultSlot).setY(y - ((AbstractContainerScreenAccessor) screen).getGuiTop() + 1 + 24);
	}

	private boolean isHoveringRedCross(int mouseX, int mouseY) {
		Slot additionalSlot = getContainer().getAdditionalSlot();
		int arrowX = getArrowX(additionalSlot.x + ((AbstractContainerScreenAccessor) screen).getGuiLeft(), getContainer().getResultSlot().x + ((AbstractContainerScreenAccessor) screen).getGuiLeft());
		int arrowY = getArrowY(additionalSlot.y + ((AbstractContainerScreenAccessor) screen).getGuiTop());
	...
	private boolean isHoveringEmptySlot(Slot slot, int mouseX, int mouseY) {
		return mouseX >= slot.x + ((AbstractContainerScreenAccessor) screen).getGuiLeft() && mouseX < slot.x + ((AbstractContainerScreenAccessor) screen).getGuiLeft() + 16 && mouseY >= slot.y + ((AbstractContainerScreenAccessor) screen).getGuiTop() && mouseY < slot.y + ((AbstractContainerScreenAccessor) screen).getGuiTop() + 16 && slot.getItem().isEmpty();
	}
